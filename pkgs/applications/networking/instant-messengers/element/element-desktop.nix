...
let
  ...
  linux = stdenv.mkDerivation (finalAttrs: builtins.removeAttrs pinData [ "hashes" ] // {
    pname = pname;
  ...
  darwin = stdenv.mkDerivation (finalAttrs: builtins.removeAttrs pinData [ "hashes" ] // {
    pname = pname;

    src = fetchurl {
      url = "https://packages.element.io/desktop/update/macos/Element-${finalAttrs.version}-universal-mac.zip";
      hash = "sha256-HoAVFXwfXmX5PziyyqEc/65ib7ouh4MozlgoCAHuDi4=";
    };
    ...
    installPhase = ''
      runHook preInstall
      mkdir -p "$out/Applications/Element.app"
      cp -R . "$out/Applications/Element.app"
      runHook postInstall
    '';
